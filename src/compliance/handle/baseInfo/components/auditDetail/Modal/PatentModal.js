...
class App extends Component {
    ...
    columns = [
        {
            ...
            render: (t, row) => {
                return (
                    <div>
                        <p>注册国家：{row.registerCountry}</p>
                        <p>专利号：{row.patentId}</p>
                        <p>权利人：{row.trademarkNumber}</p>
                    </div>
                )
            }
        }, {
            title: '专利图片',
            key: 'patentInfo',
            dataIndex: 'patentInfo',
            width: 60,
            render: (text) => {
                if (text && Array.isArray(text) && text.length) {

                    return text.map(v => {
                        if (v.patentPic && Array.isArray(v.patentPic) && v.patentPic.length) {
                            return v.patentPic.map(val => (<img style={{ width: "100%" }} src={val} alt="专利图片" />))
                        }
                        return null
                    })
                }
            ...
            dataIndex: 'patentPic',
            width: 60,
            render: t => <img style={{ width: "100%" }} src={t} alt="实物图片" />
        }, {
            title: '操作',
            key: 'reviewers',
            align: 'center',
    ...
    listFetch = (pageNumber, pageData) => {
        ...
        fetchPost('/irp/api/GetPatentPoolList/getPatentPoolList', params)
            .then(data => {
        ...
        return (
            <div className="baseinfo-detail-modal">
                <div className="baseinfo-detail-modal-search">
                    <Input placeholder="请输入专利名称" onChange={this.setName} />
