...
// import { VERSIONPOOL_EDIT, 
//          VERSIONPOOL_DELETE } from "../constants";
...
class App extends Component {
    ...
    handleOk = ()=> {
        this.refs.form.validateFields((err, values) => {
            ...
            if (this.state.newVisible) {
                ...
                params.versionId = this.state.currentItem.versionId
                this.postEditRequest(params)
            }
        });
    }
    ...
    postEditRequest(params) {
        fetchPost(path.irp +'VersionRepositoryAddOrEdit/versionRepositoryAddOrEdit', params).then(data=>{
    ...
    showDeleteConfirm = (item) => {
        const params = { versionId: item.versionId }
        const self = this
        confirm({
            maskClosable: true,
            title: '确定要删除吗？',
            ...
            cancelText: '取消',
            onOk() {
                fetchPost(path.irp + 'VersionRepositoryDelete/versionRepositoryDelete', params).then(data =>{
                    if(data && data.state === "000001"){
                        message.success('操作成功.');
                        self.listFetch();
                    } else {
    ...
    renderDetailComponent() {
        if (this.state.newVisible) {
            ...
            return (<Detail ref="form" getRef={this.getRef} item={this.state.currentItem}/>)
        }
    }
    render() {
        ...
        return (
            <Functions { ...this.props } isPage={true} functionkey="007-000001-000004-001">
