...
export default class SearchComponent extends React.Component {
    ...
    componentDidMount() {
        this.setDefaultTime();
        // 获取库存状态数据
        this.props.queryCapacityState();
        // 获取发货优先级数据
        this.props.queryPriorityType();
    }
    ...
    render() {
        ...
        const { handleChange, paramePlatformId, toggleModal, onSearch, onReset, capacitystatedata, prioritytypedata } = this.props;
        const selectSearch = (
            ...
            <Row type="flex" align="middle">
                <Col span={24}>
                    <FormItem
                        label="发货优先级"
                    >
                        {getFieldDecorator('deliveryPriority', {
                            initialValue: ['']
                        },
                        )(
                            <Ctags
                                list={prioritytypedata}
                ...
                <Row type="flex" align="middle">
                    <Col span={24}>
                        <FormItem
                            label="库存状态"
                        >
                            {getFieldDecorator('stockState', {
                                initialValue: ['']
                            },
                            )(
                                <Ctags
                                    list={capacitystatedata}
                                    handleChange={() => onSearch()}
                                />
                            )}
                        </FormItem>
