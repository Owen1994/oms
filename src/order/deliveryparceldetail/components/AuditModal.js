...
export default class AuditModal extends React.Component {
    ...
    render() {
        const { visible, handleAudit } = this.props;
        const { warehouseOrderState } = this.props.Infos;
        const { getFieldDecorator, getFieldValue } = this.props.form;
        const { loading } = this.state;
        const ifShowPass = warehouseOrderState && warehouseOrderState.value !== '待审核' ? false : true;
        return (
            <Modal
                visible={visible}
                title={'审核'}
                        ...
                        >
                            {
                                getFieldDecorator('auditStatus', {
                                    initialValue: ifShowPass ? 2 : 0
                                })(
                                    <RadioGroup onChange={() => {this.setState({ loading: false })}}>
                                        {
                                            ifShowPass ? <Radio key={2} value={2}>通过</Radio> : null
                                        }
