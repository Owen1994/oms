...
import { ADD_NEW } from '../constants/Api';
import {
    GET_USER_WAREHOUSE,
...
class AddModal extends React.Component {
    ...
    handleSubmit = () => {
        this.props.form.validateFields((err, values) => {
            ...
            if (!err) {
                fetchPost(ADD_NEW, { data: values }, 1)
                    .then((result) => {
                        if (result.state === '000001') {
                            this.handleCancel();
                ...
                >
                    <Form>
                        <div className="wms-modal-form">
                            ...
                            >
                                {
                                    getFieldDecorator('deliveryCodeNumber', {
                                        rules: [{ required: true, message: '交付码编号不能为空' }],
                                    })(
                                        <Input
                                            placeholder="请输入"
                                            onChange={() => {
                            ...
                            >
                                {
                                    getFieldDecorator('packingTableNumber', {
                                        rules: [{ required: true, message: '打包台号不能为空' }],
                                    })(
                                        <Input
                                            placeholder="请输入"
                                            onChange={() => {
