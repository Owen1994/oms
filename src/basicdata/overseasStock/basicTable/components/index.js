...
import { parseStrToArray, parseStrToChineseArray } from '../../../../util/StrUtil';
...
class Retrieval extends React.Component {
    ...
    handleSearch = (pageNumber = 1, pageData = 20) => {
        ...
        if (formData.searchContent) {
            if (formData.searchType === 3) {
                formData.searchContent = parseStrToChineseArray(formData.searchContent);
            } else {
    ...
    handleExport = () => {
        ...
        if (data.searchContent) {
            data.searchContent = parseStrToArray(data.searchContent);
            if (data.searchContent.length >= 100) {
    ...
    render() {
        ...
        return (
            <Functions {...this.props} isPage functionkey="011-000002-000001-001">
