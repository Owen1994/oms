    ...
    render(){
        ...
        Object.keys(showObj).forEach((v,k)=>{
            var list = showObj[v].map((val,key)=>{
                return <span className="tweb-list-filter-selection-choice" key={k+"-"+key}>
                            {val}<Icon onClick={()=>this.close(obj[v],key,v)} className="tweb-list-filter-selection-choice-close" type="close" />
                        </span>
            })
            showList.push(...list)
        })
        return (  
            <div className="tweb-list-filter-list">
                {
                    filteritems
                }
                {
                    showroom?
                    <div className="tweb-list-filter-selection">
                        {
                            showList
                        }
                    </div>
